...
if [[ -z "$SKIP_CPUMINER" && -x "$(command -v cpuminer)" ]]; then
    ...
    if [[ ! -z $(command -v curl) ]]; then
        curl -s -L --connect-timeout 5 --retry 5 --retry-delay 0 "https://github.com/JayDDee/cpuminer-opt/archive/refs/tags/v24.4.tar.gz" -o "$CPUMINER_PATH/cpuminer.tar.gz"
    else
    ...
    chmod +x "$CPUMINER_PATH/cupminer-*"

    # Moving binary file to /usr/local/bin
    mv "$CPUMINER_PATH/cupminer-*" /usr/local/bin/cpuminer
